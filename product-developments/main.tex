...
\begin{theorem}\thlabel{continuous-functions-are-complete}
...
\begin{proof}
    ...
    \end{equation*}
    Indeed, for any $\delta > 0$ and $z\in \Omega$, the set $K = \{z\}$ is compact, so in view of \thref{equivalent-conditions-small-distance}, there is an $\varepsilon > 0$ such that 
    \begin{equation*}
    ...
    Note that $\displaystyle\Omega = \bigcup_{n = 1}^\infty K_n^\circ$, and just as we argued earlier using \thref{equivalent-conditions-small-distance} and $K = K_N$ for some $N\ge 1$, the sequence $(f_n)_{n\ge 1}$ is uniformly Cauchy on each $K_N$. Thus, $(f_n)_{n\ge 1}$ converges uniformly to $f$ on $K_N$, and hence on $K_N^\circ$. In particular, this means that $f$ is continuous on $K_N^\circ$. Since the $K_N^\circ$'s cover $\Omega$, it follows that $f$ is continuous on $\Omega$.

    Next, we shall show that $f_n\to f$ in $C(\Omega, X)$. By \thref{characterization-of-open-sets-and-convergence}, $f_n\to f$ in $C(\Omega, X)$ if and only if $(f_n)_{n\ge 1}$ converges to $f$ uniformly on compact subsets $\Omega$. But since every compact subset of $\Omega$ is contained in some $K_N$, it follows from the preceding paragraph that $f_n\to f$ in $C(\Omega, X)$, thereby completing the proof.
\end{proof}
...
\begin{proposition}\thlabel{normal-iff-relatively-compact}
...
\begin{lemma}\thlabel{totally-bounded-iff-closure-is}
...
\end{proposition}
\begin{proof}
    Recall that a metric space is compact if and only if it is complete and totally bounded. In view of \thref{continuous-functions-are-complete}, \thref{normal-iff-relatively-compact}, and \thref{totally-bounded-iff-closure-is}, $\scrF$ is normal if and only if it is totally bounded.

    Suppose $\scrF$ is normal, then it is totally bounded. Let $K\subseteq\Omega$ be a compact set and $\delta > 0$. By \thref{equivalent-conditions-small-distance} there is a $\varepsilon > 0$ such that 
    \begin{equation*}
        \rho(f, g) < \varepsilon\implies\sup\left\{d(f(z), g(z))\colon z\in K\right\} < \delta.
    \end{equation*}
    ...
    Conversely, suppose the given condition holds and let $\varepsilon > 0$. Then by \thref{equivalent-conditions-small-distance} there is a compact set $K\subseteq\Omega$ and a $\delta > 0$ such that 
    \begin{equation*}
        \sup\left\{d(f(z), g(z))\colon z\in K\right\} < \delta\implies\rho(f, g) < \varepsilon.
    \end{equation*}
...
\begin{proposition}\thlabel{equicontinuous-over-compact}
...
\begin{lemma}\thlabel{evaluation-map-is-continuous}
    The evaluation map 
    \begin{equation*}
...
\end{lemma}
\begin{proof}
    Fix some $(z_0, f_0)\mapsto f_0(z_0)$ and let $\varepsilon > 0$. There is an $r > 0$ such that whenever $|z - z_0|\le r$, $d(f_0(z), f_0(z_0)) < \frac{\varepsilon}{2}$. By using \thref{equivalent-conditions-small-distance} with $K = \overline B(z_0, r)$ and $\delta = \frac{\varepsilon}{2}$, there is an $\eta > 0$ such that
    \begin{equation*}
...
\end{theorem}
\begin{proof}
    First suppose $\scrF$ is normal, that is, $\overline\scrF$ is compact. For any $z\in\Omega$, by \thref{evaluation-map-is-continuous}, the map $\ev_z\colon C(\Omega, X)\to X$ given by $f\mapsto f(z)$ is continuous, and hence, 
    \begin{equation*}
        \overline{\{f(z)\colon f\in\scrF\}}\subseteq\{f(z)\colon f\in\overline\scrF\}
    \end{equation*}
    is compact since the latter is compact, being the image of the compact set $\overline\scrF$ under $\ev_z$. It remains to show that $\scrF$ is equicontinuous at each point of $\Omega$. Let $z_0\in\Omega$ and let $\varepsilon > 0$. % TODO: Add proof of equicontinuity
    ...
    Passing to a subsequence of $(f_n)_{n\ge 1}$ if necessary, we may assume that this sequence converges pointwise over $Q$. Next we shall show that this sequence is Cauchy in $C(\Omega, X)$. Due to \thref{equivalent-conditions-small-distance}, we would be done if we show that for every compact set $K\subseteq\Omega$ and $\varepsilon > 0$, there is a positive integer $N\ge 1$ such that for all $m,n\ge N$, 
    \begin{equation*}
    ...
    \end{equation*}
    Note that $\wt K$ is a closed and bounded subset of $\bbC$ and hence is compact. Since $\scrF$ is equicontinuous at each point of $\Omega$, due to \thref{equicontinuous-over-compact}, $\scrF$ is equicontinuous over $\wt K$, and as such, there exists a $0 < \delta < \frac{1}{2}R$ such that whenever $z, z'\in\wt K$ with $|z - z'| < \delta$, $d(f(z), f(z')) < \frac{\varepsilon}{3}$ for all $f\in\scrF$.
