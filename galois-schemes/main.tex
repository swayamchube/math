...
\end{proposition}
\begin{proof}
    Let $Y = X^{\# F(X)}$ be the product of $\# F(X)$ copies of $X$. As $F$ commutes with products, we have $F(Y) = F(X)^{\# F(X)}$. Let us index the coordinates of $Y$ by the elements of $F(X)$, and let $a\in F(Y)$ % TODO: Complete this
\end{proof}
