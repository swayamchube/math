...
\begin{theorem}[Leray]\thlabel{thm:leray}
...
\begin{theorem}\thlabel{thm:finiteness-of-cohomology}
...
Since $X$ is compact, there is a finite covering $\frakV = \{V_\alpha\}$ such that each $V_\alpha$ is an \emph{analytic disc}\footnote{An open set $U\subseteq X$ is said to be a analytic disc if there is a chart $\phi: U\to D$ where $D$ is the open unit disc in $\bbC$.} and such that $\frakV$ is a refinement of $\frakU$. Due to \thref{thm:leray}, we know that $H^1(\frakU,\scrO)\cong H^1(\frakV,\scrO)$ and hence, it suffices to show that the latter is a finite-dimensional vector space.
...
\begin{lemma}\thlabel{lem:preimage-bounded-boundaries}
...
\begin{lemma}\thlabel{lem:intermediate-finiteness}
...
\end{lemma}
\begin{proof}
    Choose $\rho$ such that $r < \rho < 1$. Since $\frakU(r)$ is a refinement of $\frakU$ through the identity map $\tau: I\to I$, due to \thref{thm:leray}, we have that the restriction map 
    \begin{equation*}
    ...
    \end{equation*}
    since $U_i(r)\cap U_j(r)$ is relatively compact in $U_i\cap U_j$. From \thref{lem:preimage-bounded-boundaries}, we know that the preimage of the above map under $\delta$ lies inside $C^0_b(r)$, as desired.
    ...
    \end{equation*}
    Note that $U_i(r)\cap U_j(r)$ is relatively compact in $U_i(\rho)\cap U_j(\rho)$, whence by \thref{thm:montel} it is easy to argue that $v$ is compact\footnote{Probably add more details here?}. 

    Finally, by \thref{thm:schwartz}, the continuous linear mapping $u + v : Z^1_b(\rho)\oplus C^0_b(r)\to Z^1_b(r)$ given by 
    \begin{equation*}
...
\begin{lemma}\thlabel{lem:U(r)-finiteness}
    $\dim H^1\left(\frakU(r),\scrO\right) < \infty$.
\end{lemma}
\begin{proof}
    Let $\tau: I\to I$ be the identity map. Due to \thref{thm:leray}, the induced map $\tau^\ast: H^1(\frakU,\scrO)\to H^1(\frakU(r),\scrO)$ is an isomorphism. The map 
    \begin{equation*}
    ...
    \end{equation*}
    induces a map $\beta: Z^1_b(\rho)/B^1_b(\rho)\to H^1(\frakU(r),\scrO)$ such that $\tau^\ast = \beta\circ\alpha$. In particular, this means that $\beta$ is surjective. Due to \thref{lem:intermediate-finiteness}, $H^1(\frakU(r),\scrO)$ is finite-dimensional.
\end{proof}

\begin{proof}[Proof of \thref{thm:finiteness-of-cohomology}]
    Follows immediately from \thref{thm:leray} and \thref{lem:U(r)-finiteness}.
\end{proof}

\begin{definition}
    As a consequence of \thref{thm:finiteness-of-cohomology}, we have that for a compact Riemann surface $X$, $\dim H^1(X,\scrO) < \infty$ and this integer is called the \define{genus} of $X$.
\end{definition}

\subsection{The Riemann Existence Theorem}

\begin{theorem}[Riemann]\thlabel{thm:riemann-existence}
...
\end{corollary}
\begin{proof}
    For every pair $i\ne j$, using \thref{thm:riemann-existence}, one gets a function $f\in\scrM(X)$ which has a pole at $a_i$ but is holomorphic at $a_j$. Choose a constant $\lambda_{ij}\in\bbC^\times$ such that 
    \begin{equation*}
