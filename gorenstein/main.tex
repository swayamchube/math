...
\begin{lemma}[Depth Lemma]\thlabel{lem:depth-lemma}
...
\begin{proof}
    ...
    Now, since $\injdim_S S = d < \infty$, using \thref{thm:ischebeck-formula}, we have 
    \begin{equation*}
    ...
    \begin{equation*}
        0\to N\to F_0\to M\to 0.
    \end{equation*}
    Due to \thref{lem:depth-lemma}, since $\depth_S M = \depth_S F_0$,
    \begin{equation*}
    ...
    \end{equation*}
    Breaking up the above long exact sequence into short exact sequences and making repeated use of \thref{lem:depth-lemma}, it is clear that $\depth_S M^\ast = \dim S$, and hence, $M^\ast$ is a maximal Cohen-Macaulay $S$-module.
\end{proof}
\end{document}