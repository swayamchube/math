...
\begin{theorem}\thlabel{thm:extension-valuation}
...
\end{theorem}
\begin{proof}
    % TODO: Add in. All the lemmas are already there
\end{proof}

\subsection{Extensions of Local Fields}

\begin{definition}
    Let $L/K$ be a finite extension of local fields. Let $v: K^\times\to\Z$ denote the exponential valuation of $K$ and $w: L^\times\to\Q$ the unique extension of $v$ to $L$. From \thref{thm:extension-valuation}, we know that 
    \begin{equation*}
        w(\alpha) = \frac{1}{n}v\left(N^L_K(\alpha)\right)\qquad\forall~\alpha\in L.
    \end{equation*}
    The \define{ramification index} of this extension is defined to be
    \begin{equation*}
        e\coloneq \left[w\left(L^\times\right) : v\left(K^\times\right)\right],
    \end{equation*}
    and the \define{inertia degree} is defined to be $[\lambda : \kappa]$, where $\lambda$ and $\kappa$ are the residue fields of $L$ and $K$ respectively.
\end{definition}
...
\begin{proposition}[Fundamental Identity]\thlabel{prop:fundamental-identity}
    Let $L/K$ be a finite extension of local fields of ramification index $e$ and inertia degree $f$. Then $[L : K] = ef$.
\end{proposition}
\begin{proof}
    % TODO: Add in later. It's a bit long
\end{proof}

\begin{definition}
    A finite extension $L/K$ of local fields is said to be \define{unramified} if $e(L/K) = 1$, or equivalently, in light of \thref{prop:fundamental-identity}  $[L : K] = [\lambda :\kappa]$.
\end{definition}
